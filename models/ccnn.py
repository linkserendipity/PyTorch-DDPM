...
class CCNN_emb(nn.Module):
    r'''
    返回 x, emb ([256, 2]) ([256, 1024])
    ...
    def __init__(self, in_channels: int = 4, grid_size: Tuple[int, int] = (9, 9), num_classes: int = 2, dropout: float = 0.5):
        ...
        self.lin1 = nn.Sequential(
            ...
            nn.Dropout(self.dropout) #TODO 改成dropout???
            # nn.Dropout2d(self.dropout) #TODO 改成dropout???
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.lin1(x)
        emb = x
        #TODO 最后一层 1024维的 feature vectore 提出来 还要和label 对应上
